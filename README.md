...
[![Moodle Plugin CI](https://github.com/engfeda-ui/competency-report/actions/workflows/ci.yml/badge.svg?branch=master)](https://github.com/engfeda-ui/competency-report/actions/workflows/ci.yml)
...
[![Version](https://img.shields.io/badge/Version-v3.25.1-blue.svg?style=flat-square)](https://github.com/engfeda-ui/competency-report)
...
### v3.25.1 — 2026-09-08
- **CodeSniffer & CI Compliance:**
  - Converted deprecated `list()` syntax to short array destructuring syntax `[...]` in `school_report.php` and `school_pdf.php` to adhere to Moodle coding standards (`Universal.Lists.DisallowLongListSyntax`).
  - Removed blank line immediately following class opening brace in `tests/competency_calculator_test.php` to satisfy PSR-12 code style (`PSR12.Classes.OpeningBraceSpace`).
...
- **Feature (Modern Institutional Competency Dashboard & Executive PDF):** Overhauled `school_report.php` and `school_pdf.php` into an executive, high-performance Institutional Dashboard:
  - **Executive KPI Cards:** Evaluated active courses count, unique evaluated students, overall institutional mastery rate, and assessed competencies count.
  - **Visual Analytics:** Added Chart.js course comparison bar chart and 3-tier competency mastery distribution doughnut chart.
  - **Focus Competencies:** Instant identification of top 5 achieving competencies and lowest 5 competencies requiring support with visual progress indicators.
  - **Active Courses Master Roster:** Multi-metric course breakdown with live instantaneous search, theory & practical mastery rates, category badges, direct links to course master reports, and client-side CSV table export.
  - **Executive PDF Export:** Redesigned `school_pdf.php` with official header, KPI tables, and color-coded course matrix.
...
### v3.19.31 — 2026-08-26
- **Data integrity (Migration @ 2026082700):** Practical results table now enforces a **UNIQUE** index on `(assessmentid, studentid, competencyid)`. Existing duplicates are removed during upgrade (keeping the newest entry per group), and `install.xml` updated for fresh installs.
- **Fix:** Concurrent practical-entry saves no longer create duplicate rows — insert races are caught and converted to updates (`practical_entry.php`).
- **Performance/Memory:** `purge_duplicate_evidence()` rewritten as a self-joining ID query — only duplicate evidence IDs are fetched from the database instead of streaming the entire site-wide evidence table through PHP.

### v3.19.30 — 2026-08-26
- **Fix & Reliability (Recordset Stream Processing for Bulk Rates):** Replaced `$DB->get_records_sql()` with `$DB->get_recordset_sql()` in `get_quiz_rates_bulk()` and `get_practical_rates_bulk()` inside `classes/competency_calculator.php`. Prevents row-key collisions during multi-user bulk preloading, guaranteeing complete and accurate score aggregation.

### v3.19.29 — 2026-08-26
- **Performance Optimization (N+1 Query Elimination & Bulk Rate Preloading):** Implemented `preload_user_data()` with `get_quiz_rates_bulk()` and `get_practical_rates_bulk()` in `classes/competency_calculator.php`, preloading quiz and practical competency scores across all enrolled students in two consolidated queries instead of per-student roundtrips.
- **Performance & Query Scoping:** Scoped `{question_attempt_steps}` subqueries in `lib.php` (`local_comp_report_ext_build_context_details`) to the specific course and user attempts, preventing expensive full-table scans across all historical question attempts.

### v3.19.28 — 2026-08-26
- **Fix (Critical):** Defined missing `$now` variable in `practical_entry.php` POST handler — inserts/updates previously wrote NULL timestamps.
- **Fix:** Corrected `upgrade_plugin_savepoint()` component name in `db/upgrade.php` from `competency_report` to `comp_report_ext` (three occurrences), restoring proper incremental upgrade bookkeeping.
- **Fix:** Rebuilt stale AMD build for `modal_viewer` (`amd/build/modal_viewer.min.js`) to match the vanilla-DOM source implementation (previous build still shipped the old jQuery-based code).
- **Security:** AI provider API key setting now uses `admin_setting_configpasswordunmask` instead of plain text field.
- **Maintenance:** Removed duplicate `$string['competency']` language key from EN and AR packs.

### v3.19.27 — 2026-08-26
- **Security (Critical):** Fixed IDOR in `assessment_setup.php` update action — record updates are now scoped to the selected course (`courseid` check), preventing editing teachers from modifying other courses' assessments.
- **Security:** Hardened authorization for external WS functions `generate_ai_comment` and `generate_study_plan` — users may now only access their own report unless they hold `viewreports` capability (previously any enrolled user could read other students' grades via crafted WS calls). Mirrors the legacy AJAX endpoints.
- **Fix:** Corrected external function type from `read` to `write` in `db/services.php` — both functions trigger competency sync writes.
...
- **Cleanup:** Purged legacy `.zcode` session planning files from repository.
...
### v3.19.20 — 2026-08-23
- **Bug Fix (Exact Scaled Grade Consistency in Final Recorded Grade):** Fixed 0.01 fractional variance between Attempt 1 and Final Recorded Grade in `group_exam_analytics.php`. The final scaled grade now calculates directly from the precise attempt raw points ratio rather than multiplying the single-decimal rounded percentage, ensuring 100% mathematical consistency (e.g. `16.67 / 30` across both columns).

### v3.19.19 — 2026-08-23
- **Feature (Dual-Metric Grade & Question Item Breakdown in Exam Performance Roster):** Implemented Style A double-line structured display in `group_exam_analytics.php` and `templates/group_exam_analytics_page.mustache`. Each student score cell now prominently displays the percentage in bold, the scaled official grade out of the course maximum grade (e.g. `16.67 / 30`) in primary blue, and the exact question item count (e.g. `(25/45 Qs)` / `(25/45 أسئلة)`) as statistical context below.
...
### v3.19.4 — 2026-08-21
- **Bug Fix (Practical Exam Grades Query in External AI Service):** Corrected column names in `classes/external/ai.php` when querying `{local_comp_report_ext_prac}` for practical assessment results (`focustype = grades`). The query previously used non-existent column names (`grade`, `asmtid`, `userid`), causing a `dml_read_exception`. Now correctly queries `AVG(competency_percent)` with `assessmentid = :asmtid` and `studentid IN (...)`.

### v3.19.3 — 2026-08-21
- **Bug Fix (SQL Join in Competency Calculator):** Fixed missing `{quiz}` table join in `classes/competency_calculator.php` (`get_student_scores()`). The preload query previously filtered by `quiza.course = :courseid`, but `{quiz_attempts}` does not have a `course` column, causing a `dml_read_exception` (`Unknown column 'quiza.course' in 'WHERE'`). Now correctly joins `{quiz} q ON q.id = quiza.quiz` and filters by `q.course = :courseid`.
...
- **UI Enhancement:** Removed redundant duplicate percentage symbols `(%) (%)` in form labels and table headers, and updated backend rounding precision to 2 decimal places.

### v3.19.8 (2026082108) — 2026-08-21
- **Feature & Fix (Assessment Setup Type Toggling):** Fixed issue where switching Assessment Type to "Practical Assessment" in `assessment_setup.php` failed to toggle the selector from Quiz to Assignment (`mod_assign`). Built and compiled missing `amd/build/assessment_setup.min.js` and `assessment_setup.min.js.map`, updated `amd/src/assessment_setup.js`, and added immediate zero-latency inline event fallback handlers to `templates/assessment_setup_page.mustache`.
- **UX Enhancement (Auto-Fill Assessment Name):** Restored and enhanced automatic population of the Assessment Name field whenever a Quiz or Assignment activity is selected from the dropdowns.
- **Code Optimization:** Simplified assignment verification check in `assessment_setup.php` using `$DB->record_exists('assign', ['id' => $assignid, 'course' => $courseid])`.
